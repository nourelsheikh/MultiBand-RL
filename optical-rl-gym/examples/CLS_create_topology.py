    ...
    for idn1, n1 in enumerate(topology.nodes()):
        for idn2, n2 in enumerate(topology.nodes()):
            if idn1 < idn2:
                ...
                objs = []
                for path, length, modulationC, modulationL, modulationS in zip(paths, lengths, selected_modulationsC, selected_modulationsL, selected_modulationsS):
                        #if (modulationC['band'] == 'L'):
                          #print("*&^%^&*((&*&%$^&")
                        #print(idp, modulationC, "MOD C")
                        #print(idp, modulationL, "MOD L")
                        objs.append(Path(idp, path, length, modulationC, modulationL, modulationS))      # Add to path class best_modulationC
                        print('\t', idp, length, modulationC, modulationL, modulationS, path)
...
# maximum_distance: km
#C- Band
modulations.append({'modulation': 'C-BPSK', 'capacity': 23., 'maximum_length': 17400, 'band': 'C'})  #added item in list to denote band
modulations.append({'modulation': 'C-QPSK', 'capacity': 46., 'maximum_length': 8700, 'band': 'C'})
modulations.append({'modulation': 'C-8QAM', 'capacity': 69., 'maximum_length': 4700, 'band': 'C'})
modulations.append({'modulation': 'C-16QAM', 'capacity': 92., 'maximum_length': 2300, 'band': 'C'})
modulations.append({'modulation': 'C-32QAM', 'capacity': 115., 'maximum_length': 1200, 'band': 'C'})
modulations.append({'modulation': 'C-64QAM', 'capacity': 140., 'maximum_length': 600, 'band': 'C'})
...
#S- Band
modulations.append({'modulation': 'S-BPSK', 'capacity': 23., 'maximum_length': 14800, 'band': 'S'})    #L band modulations
modulations.append({'modulation': 'S-QPSK', 'capacity': 46., 'maximum_length': 7400, 'band': 'S'})
modulations.append({'modulation': 'S-8QAM', 'capacity': 69., 'maximum_length': 4100, 'band': 'S'})
modulations.append({'modulation': 'S-16QAM', 'capacity': 92., 'maximum_length': 2000, 'band': 'S'})
modulations.append({'modulation': 'S-32QAM', 'capacity': 115., 'maximum_length': 1000, 'band': 'S'})
modulations.append({'modulation': 'S-64QAM', 'capacity': 140., 'maximum_length': 500, 'band': 'S'})
modulations.append({'modulation': 'S-256QAM', 'capacity': 186., 'maximum_length': 100, 'band': 'S'})
...
topology = get_topology('/home/zceened/optical-rl-gym/examples/topologies/nsfnet_chen.txt', 'NSFNET', modulations, k_paths=k_paths)

with open(f'./topologies/NSFNET_chen_eon_{k_paths}-paths_CLS.h5', 'wb') as f:
